package com.ubtechinc.cruzr.calendar.datepicker.bizs.calendars;
...
public class DPCNCalendar extends DPCalendar {
        ...
        for (int i = 0; i < tmp.length; i++) {
            for (int j = 0; j < tmp[0].length; j++) {
                ...
                if (!TextUtils.isEmpty(gregorianMonth[i][j])) {
                    g.y = year;
                    g.m = month;
                    g.d = Integer.parseInt(gregorianMonth[i][j]);
                    L l = null;
                    String result = "";
    ...
    public boolean isDeferred(int year, int month, int day) {
        if (year == 2015) {
            ...
            for (String s : deferredOfMonth) {
                if (!TextUtils.isEmpty(s) && Integer.parseInt(s) == day) return true;
            }
        }
        return false;
    }
    ...
    private String getSolarTerm(int year, int month, int day) {
        ...
        if (Integer.parseInt(STOfMonth[0]) == day) {
            return SOLAR_TERM[month - 1][0];
        } else if (Integer.parseInt(STOfMonth[1]) == day) {
