...
import com.ubtechinc.cruzr.calendar.datepicker.entities.DPInfo;
...
//                        c.isToday(year, month, Integer.valueOf(tmp.strG));
                        c.isToday(year, month, Integer.parseInt(tmp.strG));
                if (strWeekend.contains(tmp.strG)) tmp.isWeekend = true;
                if (c instanceof DPCNCalendar) {
                    if (!TextUtils.isEmpty(tmp.strG)) tmp.isSolarTerms =
                            ((DPCNCalendar) c).isSolarTerm(year, month, Integer.parseInt(tmp.strG));
